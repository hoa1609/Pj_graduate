@if(isset($widgets['group-product2']) && !is_null($widgets['group-product2']))
    @foreach ($widgets['group-product2']->object as $category)
        @php
            $catName = $category->languages->first()->pivot->name;
            $catCanonical = write_url($category->languages->first()->pivot->canonical, true, true);
            $childrens = ($category->childrens) ?? null ;
        @endphp
        <section class="section ec-product-tab section-space-p">
            <div class="container">
                <div class="col-lg-12 col-md-12">
                   <!------danh muc san pham------->
                    @if(isset($childrens) && !is_null($childrens))
                        <div class="row space-t-50">
                            <div class="uk-flex uk-space-between px-2">
                                <div class="section-title">
                                    <h2 class="ec-title">{{ $catName }}</h2>
                                </div>
                                <div class="ec-pro-tab">
                                    <ul class="ec-pro-tab-nav nav justify-content-end">
                                        <li class="nav-item"><a class="nav-link active" href="{{ $catCanonical }}">Tất cả</a></li>
                                        @foreach ($childrens as $children)
                                            @php
                                                $chilName = $children->languages->first()->pivot->name;
                                                $chilCanonical = write_url($children->languages->first()->pivot->canonical, true, true);
                                            @endphp
                                            <li class="nav-item"><a class="nav-link active" href="{{ $chilCanonical }}">{{ $chilName }}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                        </div>
                    @endif
                   <!------san pham--->
                    @if(isset($category->products) && count($category->products))
                        <div class="row margin-minus-b-15" data-animation="fadeIn">
                            <div class="col">
                                <div class="tab-content">
                                    <div class>
                                        <div class="row">
                                            @foreach ($category->products as $product)
                                                <div class="col-lg-3 col-md-6 col-sm-6 col-xs-6 ec-product-content">
                                                    @include('frontend.component.product-item')
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    @endif
                </div>
            </div>
        </section>
    @endforeach
@endif