                        ...
                        <div class="row mb-4">
                            <div class="col-md-12 position-relative">
                                ...
                                <textarea 
                                    ...
                                    data-height="100"
                                    >
                                    {{ old('description', ($attributeCatalogue-> description) ?? '') }}
                                </textarea>
                            </div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-md-12 position-relative">
                                ...
                                <textarea 
                                    ...
                                    data-height="300"
                                    >
                                    {{ old('content', ($attributeCatalogue-> content) ?? '') }}
                                </textarea>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-12 position-relative">
                                ...
                                <select name="parent_id" class="form-select form-select-important">
                                    @foreach ($dropdown as $key => $val)
                                        <option {{ ($key == old('parent_id', $attributeCatalogue->parent_id ?? '')) ? 'selected' : '' }} value="{{ $key }}">{{ $val }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="card-body pt-10">
                        <div class="row">
                            ...
                            <span class="image img-cover image-target">
                                @php
                                    $image = old('image', $attributeCatalogue->image ?? ''); 
                                ...
                                <div class="mb-2">
                                    <select name="publish" class="form-select form-select-important">
                                        @foreach (config('apps.general.publish') as $key => $val)
                                            <option {{ ($key == old('publish', $attributeCatalogue->publish ?? '')) ? 'selected' : '' }} value="{{ $key }}">{{ $val }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="">
                                    <select name="follow" class="form-select form-select-important">
                                        @foreach (config('apps.general.follow') as $key => $val)
                                            <option {{ ($key == old('follow', $attributeCatalogue-> follow ?? ''))  ? 'selected' : '' }}  value="{{ $key }}">{{ $val }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="ibox-content mb-3">
                            <div class="seo-container">
                                <div class="meta-title fs-5">
                                    {{ (old('meta_title', ($attributeCatalogue-> meta_title) ?? '' )) ?? 'Đây là tiêu đề cho bài viết' }}
                                </div>
                            </div>
                            <div class="canonical mb-2">
                                {{ ( ($attributeCatalogue-> canonical) ?? '') ? config('app.url').old('canonical', ($attributeCatalogue-> canonical) ?? '').config('apps.general.suffix') : 'http://duong-dan-cua-ban.html' }}
                            </div>
                            <div class="meta-description">
                                {{ (old('meta_description', ($attributeCatalogue-> meta_description) ?? '')) ?? 'Đây là mô tả bài viết.......' }}
                            </div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-md-12 position-relative">
                                ...
                                <input class="form-control" 
                                    ...
                                    type="text" 
                                    placeholder="nhập tên bài viết..."
                                    value="{{ old('meta_title', ($attributeCatalogue-> meta_title) ?? '' ) }}"
                                    >
                            </div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-md-12 position-relative">
                                ...
                                value="{{ old('meta_keyword', ($attributeCatalogue-> meta_keyword) ?? '' ) }}"
                                >
                            </div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-md-12 position-relative">
                                ...
                                <textarea  
                                    ...
                                    placeholder="nhập mô tả"
                                >{{ old('meta_description', ($attributeCatalogue-> meta_description) ?? '') }}</textarea>
                            </div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-md-12 position-relative">
                                ...
                                <div class="input-group mb-3">
                                    ...
                                    <input 
                                        ...
                                        class="form-control"
                                        name="canonical" 
                                        value="{{ old('name', ($attributeCatalogue-> canonical) ?? '' ) }}"
                                        >
                                </div>
                            </div>
                        </div>
